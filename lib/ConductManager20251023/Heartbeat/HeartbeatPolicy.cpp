...
constexpr uint32_t HEARTBEAT_UPDATE_EPSILON_MS = 50; // Require meaningful change before rescheduling
constexpr float DIST_SMOOTH_ALPHA = 0.2f;       // Simple low-pass filter for sensor jitter
...
uint32_t distanceToHeartbeat(float mm) {
	...
	float interval = static_cast<float>(HEARTBEAT_MIN_MS) +
					 ratio * static_cast<float>(HEARTBEAT_MAX_MS - HEARTBEAT_MIN_MS);
